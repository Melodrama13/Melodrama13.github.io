...
SVG blur 由每个外层表面的计算 CSS property `--ui-glass-refraction-blur-radius` 决定：未设置、非法或负值时回退为 `GLASS_PRESET.blurRadius` 的 `2`（`feGaussianBlur stdDeviation="0.7"`），值上限为 `64`。prominent tier 把它设为 `--ui-glass-refraction-blur-prominent: 16`，因此生成 `stdDeviation="5.6"`；该值同时参与位移缓存键和 SVG filter margin，不能在 consumer 中用内联 recipe 替代。
...
4. **完成验证。** 运行第 8 节的 policy、unit、build、visual 和 Git 检查；默认视觉测试必须在不更新 baseline 的情况下通过。
5. **谨慎处理 baseline。** 只有经过审查、明确属于允许的视觉变化（本阶段是断点规范化）的改动才可以更新 visual baseline。单纯重排 CSS、抽 token、改文档或修复测试环境时，不得用 baseline 更新掩盖差异；更新前后都要记录原因和验证结果。
...
预期是 unit tests 零失败、policy 输出 `UI policy check passed`、Vite production build 成功、UI/export Playwright 全部通过且没有 baseline 更新、`git diff --check` 无输出；commit 前 `git status --short` 只能显示本次两份文档。
